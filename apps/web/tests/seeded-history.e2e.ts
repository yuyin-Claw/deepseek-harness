...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('matches the Figma context disclosure geometry', async () => {
    ...
    expect(style).toEqual({
      // Nightcity edition: the skin's light palette restyles this surface
      // (#e6ddf6 code-block fill, #7d6ca8 dimmed caption) with the same
      // geometry — a deliberate divergence from the stock Figma colors.
      backgroundColor: 'rgb(230, 221, 246)',
      borderRadius: '8px',
      color: 'rgb(125, 108, 168)',
      fontSize: '11px',
      lineHeight: '16px',
