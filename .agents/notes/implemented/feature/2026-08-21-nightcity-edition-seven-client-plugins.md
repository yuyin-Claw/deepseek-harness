...
- **theme** stacks one scheme-invariant `overrideTokens` layer instead of registering a selectable theme id. The settings-scope adoption that reverts selectable preferences never touches override layers, and both base palettes receive the same dark neon values. The atmosphere (skyline, grid, HUD brackets, scanline) is a `shell.overlay` entry.
